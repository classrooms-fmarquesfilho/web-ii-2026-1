...
| Semana | Disponível em | Vídeo | Tópicos |
|--------|---------------|-------|---------|
| 6 | 05/04 (Dom) | **Vídeo 6**: PostgreSQL + sqlc | Por que SQL vs ORM, sqlc, queries type-safe |
| 7 | 12/04 (Dom) | **Vídeo 7**: Migrations + Clean Architecture | Atlas, estrutura de camadas, inversão de dependência |
| 8 | 19/04 (Dom) | **Vídeo 8**: Testes em Go | testing package, testify, mocks, testes de integração, `errors.AsType` (Go 1.26), `testing/synctest` (estável desde Go 1.25) |
| 9 | 26/04 (Dom) | **Vídeo 9**: Autenticação JWT + OAuth 2.0 | JWT, refresh tokens, OAuth 2.0, middleware de auth |
| 10 | 03/05 (Dom) | **Vídeo 10**: Segurança - OWASP API Top 10 | Vulnerabilidades comuns, rate limiting, input validation |
...
| 12 | 17/05 (Dom) | **Vídeo 11**: Docker + CI/CD | Multi-stage builds, docker-compose, GitHub Actions, GOMAXPROCS container-aware (Go 1.25+), graceful shutdown, health checks |
| 13 | 24/05 (Dom) | **Vídeo 12**: Observabilidade | slog (GroupAttrs Go 1.25), métricas, traces, OpenTelemetry |
| 14 | 31/05 (Dom) | **Vídeo 13**: GraphQL vs REST | Quando usar GraphQL, gqlgen, queries e mutations |
| 15 | 07/06 (Dom) | **Vídeo 14**: Concorrência em Go | Goroutines, channels, sync.WaitGroup.Go(), testing/synctest, errgroup, worker pools |
| 16 | 14/06 (Dom) | **Vídeo 15**: Próximos passos | Carreira em Go, recursos avançados, comunidade |
